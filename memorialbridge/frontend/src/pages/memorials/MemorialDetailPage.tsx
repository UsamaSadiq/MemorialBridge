...
export const MemorialDetailPage = () => {
    ...
    try {
      ...
      try {
        const commentsResponse = await apiClient.get<{
          items: Array<{ id: string; content: string; user_email: string; created_at: string; is_hidden?: boolean }>;
        }>(`/memorials/${id}/comments`);
  ...
  const toImageSrc = (url: string | undefined): string => {
    if (!url) return '';
    ...
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600 text-sm">Loading memorial...</p>
        </div>
      </div>
    );
  }
  ...
  if (!memorial) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <p className="text-gray-900 text-lg mb-4">Memorial not found</p>
          <button
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header - matches Memorials list, Charities, Donations pattern */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex justify-between items-start md:items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2 shrink-0">
              {canEditMemorial() && (
                <>
                  <button type="button" onClick={handleEdit} className="btn btn-primary flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 rounded-lg border border-red-300 bg-red-50 text-red-700 font-medium hover:bg-red-100 transition-colors"
                  >
                    Delete
                  </button>
                </>
              )}
              {user?.is_admin && memorial?.status === 'pending' && (
                <>
                  <button type="button" onClick={handleApprove} className="btn btn-success flex items-center gap-2">
                    ✓ Mark as Approved
                  </button>
                  <button
                    type="button"
                    onClick={handleReject}
                    className="px-4 py-2 rounded-lg border border-gray-300 bg-gray-100 text-gray-700 font-medium hover:bg-gray-200 transition-colors"
                  >
                    Reject
                  </button>
                </>
              )}
              {user?.is_admin && memorial?.status === 'approved' && (
                <button
                  type="button"
                  onClick={() => handleSetFeatured(!memorial.is_featured)}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg border font-medium transition-colors ${
                    memorial.is_featured
                      ? 'border-amber-300 bg-amber-50 text-amber-800 hover:bg-amber-100'
                      : 'border-gray-300 bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                  title={memorial.is_featured ? 'Remove from home page featured' : 'Show on home page featured section'}
                >
                  {memorial.is_featured ? '★ Featured' : '☆ Feature for home'}
                </button>
              )}
            </div>
          </div>
          <div className="flex flex-wrap gap-3 mt-6">
            <span className="inline-flex items-center gap-1.5 px-4 py-2 bg-gray-100 text-gray-800 text-sm font-medium rounded-lg border border-gray-200">
      ...
      {/* Main Content - same card pattern as Memorials list, Charities, Donations */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Story Section */}
        {displayStory && (
          <div className="bg-white rounded-lg shadow-sm p-6 mb-6 border border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900 mb-4">Their Story</h2>
            <p className="text-gray-700 whitespace-pre-wrap leading-relaxed">{displayStory}</p>
          </div>
        )}
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6 border border-gray-200">
          ...
          <div className="flex flex-wrap gap-4">
            {(memorial.images || []).map((img) => {
              const rawUrl = 'url' in img ? img.url : (img as { image_url?: string }).image_url;
              const src = toImageSrc(rawUrl);
              return (
                <div key={img.id} className="w-40 h-40 rounded-lg overflow-hidden border border-gray-200 bg-gray-100">
                  <img
                    src={src}
                    alt="Memorial"
            ...
            {canEditMemorial() && (memorial.images?.length ?? 0) < 2 && (
              <label className="w-40 h-40 rounded-lg border-2 border-dashed border-gray-300 flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 transition-colors text-gray-600">
                <input
                  type="file"
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
          <h2 className="text-xl font-semibold text-gray-900 mb-6">Tribute Wall</h2>
          ...
          {user && (
            <form onSubmit={handleAddComment} className="mb-8 pb-8 border-b border-gray-200">
              ...
              <textarea
                ...
                placeholder="Express your feelings, share a memory, or leave a message of support..."
              ...
              <div className="mt-4 flex justify-between items-center flex-wrap gap-2">
                <p className="text-xs text-gray-600 italic">
                  Your words of remembrance are a gift to those who are grieving
                </p>
                <button
                  type="submit"
          ...
          {comments.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-600">No tributes yet. Be the first to share.</p>
              {!user && (
                <button
                  type="button"
            ...
            <div className="space-y-4">
              {comments.map((comment) => (
                <div
                  key={comment.id}
                  className="border-l-4 border-gray-300 pl-4 py-3 bg-gray-50 rounded-r-lg hover:bg-gray-100/80 transition-colors"
                >
                  <div className="flex items-center justify-between gap-2 mb-2">
                    ...
                    {user?.is_admin && (
                      <button
                        ...
                        className="text-sm text-gray-600 hover:text-red-600 transition-colors px-2 py-1 rounded border border-gray-200 hover:border-red-200"
                        title="Hide this tribute (admin)"
                      >
                        Hide
                      </button>
                    )}
                  </div>
                  <p className="text-gray-700 mt-2 whitespace-pre-wrap leading-relaxed pl-10">{comment.text}</p>
                </div>
              ))}
            </div>
          )}
